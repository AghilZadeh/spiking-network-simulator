...
class Network:
    def __init__(self):
        ...
        self.initWmat=     create_Wmat(Population.NN, Population.CP, Population.CW)              # initial weighted connectivity matrix
        self.Wmat    =     self.initWmat                                                         # this can change with plasticity
        self.Cmat    =     np.where(self.initWmat!=0, 1, 0)                                      # connectivity matrix
        self.Cpairs  =     list(zip(np.where(self.Cmat)[0],np.where(self.Cmat)[1]))              # connected pairs
...
#==============================================================================#
                    # initializing network and plasticity #
class Plasticityoff:
    def __init__(self):
        self.istrue = False   # turns plasticity off
...
def initNeuron(Pop):
    ...
    Neuron = Network()
    global Plasticity
    Plasticity = Plasticityoff()
    
def initNetwork(Pop):        # same as initNeuron
    global Population
    Population = Pop
    global Neuron
    Neuron = Network()
...
def update_Plasticity():
    global Plasticity
    global Neuron
    if Plasticity.istrue:
        Neuron.Wmat = Plasticity.updatedW()
...
def rasterplot(monitorSpiketime, monitor_ind):
    global Neuron
    if type(monitor_ind) == str:
        monitor_ind = range(Neuron.Tag.size)
...
def plot_ratePDF():
    timeinterval = Neuron.Time[-1]/1000  # elapsed time in seconds
    Rates = Neuron.SpikeCount/timeinterval    # rates in Hz
    fig, axs = plt.subplots(1,1)
    axs.set_yscale('log')
    plt.xlabel('mean rate (Hz)')
    plt.ylabel('number of neurons')
    for i, color in list(enumerate(Population.Colors)):
        sns.distplot(Rates[Neuron.Tag==i]*1, kde=False, hist_kws={"alpha":0.5, "color": color});
    # legends
    plt.legend(list(Population.Tag))
